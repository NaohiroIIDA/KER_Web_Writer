 ...
 */
export class NvmUpdiP5 extends NvmUpdi {
  ...
  async chipErase(): Promise<void> {
    ...
    await this.executeNvmCommand(NvmUpdiP5.NVMCMD_CHIP_ERASE);
    const status = await this.waitNvmReady();

    // Remove command
    await this.executeNvmCommand(NvmUpdiP5.NVMCMD_NOCMD);
  ...
  async eraseFlashPage(address: number): Promise<void> {
    ...
    await this.executeNvmCommand(NvmUpdiP5.NVMCMD_FLASH_PAGE_ERASE);
    const status = await this.waitNvmReady();
  ...
  async eraseEeprom(): Promise<void> {
    ...
    await this.executeNvmCommand(NvmUpdiP5.NVMCMD_EEPROM_ERASE);
    const status = await this.waitNvmReady();
  ...
  async writeFlash(address: number, data: Uint8Array): Promise<void> {
    await this.writeNvm(
      ...
      data,
      true,
      NvmUpdiP5.NVMCMD_FLASH_PAGE_WRITE,
      NvmUpdiP5.NVMCMD_FLASH_PAGE_BUFFER_CLEAR
    );
  }
  ...
  async writeEeprom(address: number, data: Uint8Array): Promise<void> {
    await this.writeNvm(
      ...
      data,
      false,
      NvmUpdiP5.NVMCMD_EEPROM_PAGE_ERASE_WRITE,
      NvmUpdiP5.NVMCMD_EEPROM_PAGE_BUFFER_CLEAR
    );
  }
  ...
  private async writeNvm(
    ...
    nvmcommand: number = NvmUpdiP5.NVMCMD_FLASH_PAGE_WRITE,
    erasebufferCommand: number = NvmUpdiP5.NVMCMD_FLASH_PAGE_BUFFER_CLEAR
  ): Promise<void> {
    if (!await this.waitNvmReady()) {
      throw new Error(
  ...
  private async waitNvmReady(timeoutMs: number = 100): Promise<boolean> {
    ...
    while (!timeout.expired()) {
      const status = await this.readwrite.readByte(
        this.device.nvmctrlAddress + NvmUpdiP5.NVMCTRL_STATUS
      );
      if (status & NvmUpdiP5.STATUS_WRITE_ERROR_bm) {
        throw new Error(
          `NVM error (${status >> NvmUpdiP5.STATUS_WRITE_ERROR_bp})`
        );
      }
      ...
      if (
        !(
          status &
          ((1 << NvmUpdiP5.STATUS_EEPROM_BUSY_bp) |
            (1 << NvmUpdiP5.STATUS_FLASH_BUSY_bp))
        )
      ) {
        return true;
      }
  ...
  private async executeNvmCommand(command: number): Promise<void> {
    await this.readwrite.writeByte(
      this.device.nvmctrlAddress + NvmUpdiP5.NVMCTRL_CTRLA,
      command
    );
  }
}
