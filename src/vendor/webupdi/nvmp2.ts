 ...
 */
export class NvmUpdiP2 extends NvmUpdi {
  ...
  static readonly NVMCTRL_ADDR = 0x08; // 24-bit
  ...
  // STATUS
  static readonly STATUS_WRITE_ERROR_bm = 0x30;
  ...
  async chipErase(): Promise<void> {
    ...
    await this.executeNvmCommand(NvmUpdiP2.NVMCMD_CHIP_ERASE);
    const status = await this.waitNvmReady();

    // Remove command from NVM controller
    await this.executeNvmCommand(NvmUpdiP2.NVMCMD_NOCMD);
    if (!status) {
      throw new Error(
  ...
  async eraseFlashPage(address: number): Promise<void> {
    ...
    await this.executeNvmCommand(NvmUpdiP2.NVMCMD_FLASH_PAGE_ERASE);
  ...
  async eraseEeprom(): Promise<void> {
    ...
    await this.executeNvmCommand(NvmUpdiP2.NVMCMD_EEPROM_ERASE);
    const status = await this.waitNvmReady();
  ...
  async writeEeprom(address: number, data: Uint8Array): Promise<void> {
    const nvmCommand = NvmUpdiP2.NVMCMD_EEPROM_ERASE_WRITE;
  ...
  private async writeNvm(
    ...
    const nvmCommand = NvmUpdiP2.NVMCMD_FLASH_WRITE;
  ...
  private async waitNvmReady(timeoutMs: number = 100): Promise<boolean> {
    ...
    while (!timeout.expired()) {
      const status = await this.readwrite.readByte(
        this.device.nvmctrlAddress + NvmUpdiP2.NVMCTRL_STATUS
      );
      if (status & NvmUpdiP2.STATUS_WRITE_ERROR_bm) {
        throw new Error(
          `NVM error (${status >> NvmUpdiP2.STATUS_WRITE_ERROR_bp})`
        );
      }
      ...
      if (
        !(
          status &
          ((1 << NvmUpdiP2.STATUS_EEPROM_BUSY_bp) |
            (1 << NvmUpdiP2.STATUS_FLASH_BUSY_bp))
        )
      ) {
        return true;
      }
  ...
  private async executeNvmCommand(command: number): Promise<void> {
    await this.readwrite.writeByte(
      this.device.nvmctrlAddress + NvmUpdiP2.NVMCTRL_CTRLA,
      command
    );
  }
}
